...
class ProductionTable extends DataTableComponent
{
    ...
    {
        return Production::query()->where('status', '!=', StatusProduction::WAITING_FOR_RESPONSE);
    }
            ...
            ButtonGroupColumn::make('Actions')
                ...
                ->buttons([
                    LinkColumn::make('View')
                        ...
                        ->location(fn ($row) => route('production.show', $row))
                        ->attributes(function ($row) {
                    ...
                    LinkColumn::make('Update')
                        ...
                        ->location(function ($row) {
                            return $row->status === StatusProduction::IN_PROGRESS ? route('production.update', $row) : '#';
                        })
                        ->attributes(function ($row) {
                            $class = $row->status === StatusProduction::IN_PROGRESS ? 'btn icon icon-left btn-sm btn-warning' : 'd-none btn icon icon-left btn-sm btn-warning';
